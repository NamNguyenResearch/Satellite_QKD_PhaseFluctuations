function jointProbabiliteGamma=jointProbabiliteGamma_HAP_Phase(teta_e)
    ...
    integral=quad(@calculateSigma_R_2,H_G,H_P);
    ...
    P_r_G=1./FSL.*P.*G_TX_S   .*h_p_P.*G_RX_P.*G_A.*G_TX_P   .*h_p_G.*G_RX_G.*hl   .*cos(teta_e);
    ...
    for i=1:length(t)
        ...
        for l=1:length(t)
            ...
            d0=Ei0-ScaleCo_Glo.*sqrt(sigmaN_i_l.^2);
            d1=Ei1+ScaleCo_Glo.*sqrt(sigmaN_i_l.^2);
            
            if bit_Alice_Glo==0 && bit_Bob_Glo==0
                Sum=Sum+1/2.*a(i).*special(i).^(-b(i)).*v(l).*t(l).^(b(i)-1).*qfunc((i0-d0)./sigmaN_i_l);
            elseif bit_Alice_Glo==1 && bit_Bob_Glo==0
                Sum=Sum+1/2.*a(i).*special(i).^(-b(i)).*v(l).*t(l).^(b(i)-1).*qfunc((i1-d0)./sigmaN_i_l); 
            elseif bit_Alice_Glo==0 && bit_Bob_Glo==1
    ...
    f_teta_e=1./(sqrt(2.*pi.*sigma_teta_e_2)).*exp((-teta_e.^2)./(2.*sigma_teta_e_2));
  	jointProbabiliteGamma=Sum.*f_teta_e;
end