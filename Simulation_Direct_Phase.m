function [QuantumBER_Direct_Weak,Psift_Direct_Weak,QuantumBER_Direct_Strong,Psift_Direct_Strong]=Simulation_Direct_Phase(C2n_Weak,C2n_Strong,ScaleCo_Direct_Phase)
    ...
    QuantumBER_Direct_Weak=zeros(1,length(ScaleCo_Direct_Phase));
    Psift_Direct_Weak=zeros(1,length(ScaleCo_Direct_Phase));
    
    dataTransmitted_Strong=randi([0,1],1,lengthBit);

    QuantumBER_Direct_Strong=zeros(1,length(ScaleCo_Direct_Phase));
    Psift_Direct_Strong=zeros(1,length(ScaleCo_Direct_Phase));

    for i=1:length(ScaleCo_Direct_Phase)
        ...
        for j=1:lengthBit 
            ...
            %==============================================================
            %Threshold
            d0_Weak=-1./4.*R.*M.*P_r_G.*ModDepth-ScaleCo_Direct_Phase(i)*sqrt(sigmaN_2_Weak);
            d1_Weak=1./4.*R.*M.*P_r_G.*ModDepth+ScaleCo_Direct_Phase(i)*sqrt(sigmaN_2_Weak);
            ...
            %==============================================================
            %Threshold
            d0_Strong=-1./4.*R.*M.*P_r_G.*ModDepth-ScaleCo_Direct_Phase(i)*sqrt(sigmaN_2_Strong);
            d1_Strong=1./4.*R.*M.*P_r_G.*ModDepth+ScaleCo_Direct_Phase(i)*sqrt(sigmaN_2_Strong);
