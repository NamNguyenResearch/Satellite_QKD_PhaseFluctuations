function [QuantumBER_Direct_Weak,Psift_Direct_Weak,QuantumBER_Direct_Strong,Psift_Direct_Strong]=Simulation_Direct(C2n_Weak,C2n_Strong,ScaleCo_Direct)
    ...
    QuantumBER_Direct_Weak=zeros(1,length(ScaleCo_Direct));
    Psift_Direct_Weak=zeros(1,length(ScaleCo_Direct));
    
    dataTransmitted_Strong=randi([0,1],1,lengthBit);

    QuantumBER_Direct_Strong=zeros(1,length(ScaleCo_Direct));
    Psift_Direct_Strong=zeros(1,length(ScaleCo_Direct));

    for i=1:length(ScaleCo_Direct)
        %Calculate QBER via Gamma-Gamma Channels (Weak turbulence)
        ...
        for j=1:lengthBit 
            ...
            %==============================================================
            %Variance  
            sigmaN_2_Weak=2*q*FA*(M^2)*R*(1/4*R*M*P_r_G*ModDepth*h_t_Weak+P_b_SG*G_RX_G)*deltaf+4*kB*T*Fn*deltaf/RL;
            ...
            %==============================================================
            %Threshold
            d0_Weak=-1./4.*R.*M.*P_r_G.*ModDepth-ScaleCo_Direct(i)*sqrt(sigmaN_2_Weak);
            d1_Weak=1./4.*R.*M.*P_r_G.*ModDepth+ScaleCo_Direct(i)*sqrt(sigmaN_2_Weak);
            
            
            %==============================================================
            %Calculate QBER
            iReceive_Weak(j)=iTransmitted_Weak(j)*h_t_Weak+n_t_Weak;
            ...
            %==============================================================
            %Variance  
            sigmaN_2_Strong=2*q*FA*(M^2)*R*(1/4*R*M*P_r_G*ModDepth*h_t_Strong+P_b_SG*G_RX_G)*deltaf+4*kB*T*Fn*deltaf/RL;
            ...
            %==============================================================
            %Threshold
            d0_Strong=-1./4.*R.*M.*P_r_G.*ModDepth-ScaleCo_Direct(i)*sqrt(sigmaN_2_Strong);
            d1_Strong=1./4.*R.*M.*P_r_G.*ModDepth+ScaleCo_Direct(i)*sqrt(sigmaN_2_Strong);
            
            
            %==============================================================
            %Calculate QBER
            iReceive_Strong(j)=iTransmitted_Strong(j)*h_t_Strong+n_t_Strong;
